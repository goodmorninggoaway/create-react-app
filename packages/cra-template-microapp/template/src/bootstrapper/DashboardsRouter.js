    ...
    <ScrollContainer.Scrollable>
      <Switch>
        {authz.filter(objects).map(({ url, component, label }) => (
          <Route key={url} path={buildUrl(match.path, url)} render={pageBoundaryRouteRenderer(component, label)} />
        ))}
...
export default wrapRouter(withRouter(connect(mapStateToProps)(DashboardsRouter)));
